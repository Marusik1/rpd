...
const secondDocument = {
  ...
  filename: 'Уголовное право.pdf',
  path: 'documents/bachelor/1/Уголовное право.pdf',
};
...
test('global and course search match locally by normalized name and code and recover from no results', async ({ page }) => {
  ...
  await page.getByRole('searchbox', { name: 'Поиск по курсу' }).fill('бс.2');
...
test('favorites survive reload and stale or corrupt preferences fail safely', async ({ page }) => {
  ...
  await page.reload();
  await expect(page.getByText('Здесь появятся сохранённые РПД')).toBeVisible();
...
test('recent documents render newest first and are capped at five', async ({ page }) => {
  const documents = Array.from({ length: 6 }, (_, index) => ({
    ...
    id: `document-${index}`,
    code: `БС.${index}`,
    name: `Дисциплина ${index}`,
    filename: `Дисциплина ${index}.pdf`,
    path: `documents/bachelor/1/Дисциплина ${index}.pdf`,
  }));
  ...
  await page.reload();
  const recent = page.locator('[aria-labelledby="recent-heading"] .document-row');
