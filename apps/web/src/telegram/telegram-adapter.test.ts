...
describe('Telegram adapter', () => {
  ...
  it('initializes capabilities and cleans up navigation listeners', () => { const offClick = vi.fn(); const onEvent = vi.fn(); const offEvent = vi.fn(); const app = { ready: vi.fn(), expand: vi.fn(), onEvent, offEvent, BackButton: { show: vi.fn(), hide: vi.fn(), onClick: vi.fn(), offClick } }; window.Telegram = { WebApp: app }; const adapter = createTelegramAdapter(); expect(adapter).not.toBeNull(); if (!adapter) throw new Error('Telegram adapter was not created'); expect(adapter.isTelegram).toBe(true); adapter.initialize(); const cleanupBack = adapter.setBackButton(true, vi.fn()); const onActivated = vi.fn(); const cleanupActivation = adapter.subscribeActivation(onActivated); cleanupBack(); cleanupActivation(); expect(app.ready).toHaveBeenCalled(); expect(app.BackButton.show).toHaveBeenCalled(); expect(offClick).toHaveBeenCalled(); expect(onEvent).toHaveBeenCalledWith('activated', onActivated); expect(offEvent).toHaveBeenCalledWith('activated', onActivated); });
